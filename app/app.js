...
app.controller('mainController', function ($scope, $mdDialog, $mdToast) {
    ...
    $scope.sensorFusion = sensorFusion;
    ...
    function showLoadingIndicator($mdDialog, $event, text) {
        $mdDialog.show({
            ...
            escapeToClose: false,
            template: '<md-dialog style="width: 250px;" aria-label="loading">' + '<md-dialog-content>' + '<div layout="row" layout-align="center" style="padding: 25px;">' + '</div>' + '<div layout="row" layout-align="center" style="padding-bottom: 40px;">' + '<label>' + text + '</label>' + '</div>' + '</md-dialog-content>' + '</md-dialog>',
            locals: {
                items: ''
            }
        });
    ...
    $scope.sensorFusion.disconnectIndicator = function () {
    ...
    $scope.sensorFusion.onSuccess('Connecting ....');
